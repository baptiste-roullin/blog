...
/** @returns {{ srcsetList: string[]; widthsList: number[]; }} */
function generateList(imageSettings, imageWidth, imageSrc) {
    ...
    if (imageSettings.widthsList !== undefined &&
        ...
        for (let i = 0; i < imageSettings.steps; i++) {
            let stepWidth = Math.ceil(imageSettings.minWidth +
                ((imageSettings.maxWidth - imageSettings.minWidth) /
                    (imageSettings.steps - 1)) * i);

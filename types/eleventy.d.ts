...
export interface Config {
   ...
   setUseGitIgnore(param: boolean)
   addWatchTarget(path: string)
   setWatchThrottleWaitTime(time: number)
   addNunjucksAsyncShortcode(name: string, callback: () => void)
   addLayoutAlias(name: string, path: string)
   addDataExtension(name: string, callback: (any) => any)
